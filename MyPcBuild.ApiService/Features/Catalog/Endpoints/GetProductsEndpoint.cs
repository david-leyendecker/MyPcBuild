...
namespace MyPcBuild.ApiService.Features.Catalog.Endpoints;

/// <summary>
/// Get products endpoint - retrieves a paginated list of products with optional filtering and sorting.
/// </summary>
public static class GetProductsEndpoint
{
    ...
    public static IEndpointRouteBuilder MapGetProductsEndpoint(this IEndpointRouteBuilder app)
    {
            ...
            [AsParameters] QueryParameters queryParams) =>
        {
            ...
            // Map products with HATEOAS links
            List<ProductSummary> products = productResults.Select(p => new ProductSummary(
                p.Id,
                p.Name,
                ...
                p.PublishedAt,
                [
                    linkGenerator.CreateLink($"/api/catalog/products/{p.Id}", "self", "GET"),
                    linkGenerator.CreateLink($"/api/catalog/products?category={p.ProductCategory}", "category", "GET")
                ]
            )).ToList();

            // Create response links
            List<Link> responseLinks =
            [
                linkGenerator.CreateLink($"/api/catalog/products?page={queryParams.Page}&pageSize={queryParams.ItemsPerPage}", "self", "GET"),
                linkGenerator.CreateLink("/api/catalog/categories", "categories", "GET")
            ];

            // Add pagination links
            if (pagination.HasPreviousPage)
            {
                responseLinks.Add(linkGenerator.CreateLink($"/api/catalog/products?page={queryParams.Page - 1}&pageSize={queryParams.ItemsPerPage}", "prev", "GET"));
            }

            if (pagination.HasNextPage)
            {
                responseLinks.Add(linkGenerator.CreateLink($"/api/catalog/products?page={queryParams.Page + 1}&pageSize={queryParams.ItemsPerPage}", "next", "GET"));
            }
...
// ===== Request =====
// Uses QueryParameters from Infrastructure
